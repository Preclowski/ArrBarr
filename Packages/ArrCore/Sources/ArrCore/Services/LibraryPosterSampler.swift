...
/// deck renders with `apiKey: nil` and reuses anything already in `PosterStore`.
///
...
public enum LibraryPosterSampler {
    ...
    /// the URLs AND prefetches their card-sized images, so the first
    ...
    /// memoised and `PosterStore` dedupes in-flight + serves disk/memory hits.
